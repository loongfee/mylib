...
protected:  
	...
	bool getAllFeatures();
	bool getGridFeatures(const ossimIrect& rect, radiBlockTieGptSet& tSet);
	//int runMatchNcc(const cv::Mat& slaveMat, const cv::Mat& masterMat, ossimTDpt& tDpt, void *pData, bool bDebug = false); 
	bool runMatchNcc(ossimIrect srect, double gsd_scale, ossimTDpt& tDpt, void* pData, bool bDebug = false);
...
//         ossimImageSource* source, 
   ...
   void getMaxCorrelation(ossimNCC_FFTW* &theNCCengine, cv::Mat slaveMat, cv::Mat masterMat,
	   double* pdispx, double* pdispy, double* pcor);
   ...
   OpenThreads::Mutex _cornermutex;
...
public:
	...
	virtual void run()
	{
		...
		{
			m_pRegistration->getGridFeatures(m_rectList[i], m_theTset);
			_quitmutex.lock();
			finishedBlocks++;

...
bool HJ1QVProcReader::read(const char* filename, const char* outPath)
{
	int last_pos = 0;
...
bool HJ1QVProcReader::read_band_seperate(const char* filename, const char* outPath)
{
	int last_pos = 0;
	//string strPath = SBeforeLast(string(filename), '\\');
	string strPath = string(outPath);
...
bool HJ1QVProcReader::read_by_scene_band_seperate(const char* filename, const char* outPath)
{
...
void HJ1QVProcReader::writeDescXML(const QUIVIMAGE_HEAD_INFO& headerInfo, const char* descXML)const
{
	// get a test document
	pugi::xml_document doc;
	doc.load("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");

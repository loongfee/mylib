	...
	getResidue(const vector< HJ1OptStruct >& hj1OptStructList)
{
	...
	for (int image_index = 0;image_index < nImages;++image_index)
	{
		...
		{
			//compute residue
			gd = hj1OptStructList[image_index].hj1Model->inverse((*tit)->tie);
	...
	double pstep_scale)
{
	...
	for(int i = 0;i < nImages;++i)
	{
		const vector<ossimRefPtr<ossimTieGpt> >& theTPV = hj1OptStructList[i].gcpSet->getTiePoints();
		vector<ossimRefPtr<ossimTieGpt> >::const_iterator tit;
		// loop on tie points
		for (tit = theTPV.begin() ; tit != theTPV.end() ; ++tit)
		{
			//// forward
			//// compute residue
			//resIm = (*tit)->tie - hj1OptStructList[i].hj1Model->forward(*(*tit));
			////hj1OptStructList[i].hj1Model->worldToLineSample(*(*tit), resIm);
			////resIm = (*tit)->tie - resIm;
			////hj1OptStructList[i].hj1Model->worldToLineSample(*(*tit), resIm);
			////resIm = (*tit)->tie - resIm;
			//residue(c++) = resIm.x;
			...
			//for(int p=0;p<np;p++)
			//{
			//	imDerp[p] = hj1OptStructList[i].hj1Model->getForwardDeriv( parameterList[p] , *(*tit) , pstep_scale);
			//}
			...
			// inverse
			//compute residue
			gd = hj1OptStructList[i].hj1Model->inverse((*tit)->tie);
			...
			for (int p = 0; p<np; p++)
			{
				gdDerp[p] = hj1OptStructList[i].hj1Model->getInverseDeriv(parameterList[p], (*tit)->tie, pstep_scale);
			}
...
void adjustment(const vector< HJ1OptStruct >& hj1OptStructList, const vector<int>& parameterList)
{
	...
	buildNormalEquation(hj1OptStructList, parameterList,
		...
		do
		{
			...
			if (deltap.NormFrobenius() <= minDelta) 
			{
				...
				for (int image_index = 0;image_index < nImages;++image_index)
				{
					for(int n=0;n<np;++n)
					{
						hj1OptStructList[image_index].hj1Model->setAdjustableParameter(

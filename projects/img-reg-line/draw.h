							  ...
							  const cvline_polar& l1, const cvline_polar& l2, const cv::Scalar& matchColor, int flags, int thickness=1)
{
	...
	cv::Point pt2( min((l2.pt1.x + l2.pt2.x)*0.5+outImg1.cols, double(outImg.cols-1)), (l2.pt1.y + l2.pt2.y)*0.5 );
...
{
	cv::Point2f pt1((line.pt1.x + line.pt2.x)*0.5, (line.pt1.y + line.pt2.y)*0.5);
...
{
	cv::Point2f pt1((l1.pt1.x + l1.pt2.x)*0.5, (l1.pt1.y + l1.pt2.y)*0.5);
	cv::Point2f pt2(min((l2.pt1.x + l2.pt2.x)*0.5 + outImg1.cols, double(outImg.cols - 1)), (l2.pt1.y + l2.pt2.y)*0.5);
 ...
 //////    Scalar(0, 0, 255));
	////rectangle(outImg, cv::Point(pt1.x, pt1.y) + cv::Point(0, baseline),
	////	cv::Point(pt1.x, pt1.y) + cv::Point(textSize.width, -textSize.height) - cv::Point(0, baseline),
	////	Scalar(255,255,255), CV_FILLED);
	//////line(outImg, cv::Point(pt2.x, pt2.y) + Point(0, thickness),
 //////    cv::Point(pt2.x, pt2.y) + Point(textSize.width, thickness),
 //////    Scalar(0, 0, 255));
	////rectangle(outImg, cv::Point(pt2.x, pt2.y) + cv::Point(0, baseline),
	////	cv::Point(pt2.x, pt2.y) + cv::Point(textSize.width, -textSize.height) - cv::Point(0, baseline),
